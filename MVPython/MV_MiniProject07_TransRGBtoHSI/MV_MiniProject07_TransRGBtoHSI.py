...
Intensity_picture = Image.new(imgPIL.mode, imgPIL.size)
HSI_picture = Image.new(imgPIL.mode, imgPIL.size)
...
for x in range(width):
    for y in range(height):
        ...
        Intensity_picture.putpixel((x, y), (I, I, I)) 
        HSI_picture.putpixel((x, y), (I, S, H))
...
Intensity_picture_display = np.array(Intensity_picture)
HSI_picture_display = np.array(HSI_picture)
...
Intensity_picture_display_res = cv2.resize(Intensity_picture_display, resize, interpolation = cv2.INTER_AREA)
HSI_picture_display_res = cv2.resize(HSI_picture_display, resize, interpolation = cv2.INTER_AREA)
...
cv2.imshow('Co gai Lena kenh Intensity', Intensity_picture_display_res)
cv2.imshow('Co gai Lena kenh HSI', HSI_picture_display_res)
